...
#        return
    ...
    if save_in_cam_space:
        pcd_ret = save_ply(output_path, points, rgb=None)
        ...
        if rig2world_transforms and (timestamp in rig2world_transforms):
            ...
            pcd_ret = save_ply(output_path, xyz, rgb, cam2world_transform)
            # print('Saved %s' % output_path)
        else:
            print('Transform not found for timestamp %s' % timestamp)
    return pcd_ret   # testing
...
def save_pclouds(folder,
    ...
    for path in depth_paths:
        pcds.append(save_single_pcloud(shared_dict,
                                   path,
                                   folder,
                                   pinhole_folder,
                                   save_in_cam_space,
    ...
    # save pcds in single file
    output_path = folder / 'combined.ply'
    pcd_combined = o3d.geometry.PointCloud()
    for point_id in range(len(pcds)):
        #pcds[point_id].transform(pose_graph.nodes[point_id].pose)
        pcd_combined += pcds[point_id]
    pcd_combined_down = pcd_combined.voxel_down_sample(voxel_size=0.0001)
    o3d.io.write_point_cloud(str(output_path), pcd_combined_down)
