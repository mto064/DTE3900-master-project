...
HolographicFrame^ HolographicSpatialMappingMain::Update()
{
    ...
    m_meshRenderer->m_worldCoordSystem = currentCoordinateSystem;
    ...
    if (m_surfaceAccessAllowed)
    {
        ...
        //SpatialBoundingFrustum viewFrustrum = 
        //SpatialBoundingVolume^ frustrumBoundingBox = SpatialBoundingVolume::FromFrustum(currentCoordinateSystem, viewFrustum);
...
// my methods
void HolographicSpatialMappingMain::SaveFile()
{
    StorageFolder^ localFolder = ApplicationData::Current->LocalFolder;
    //StorageFolder tempFolder = ApplicationData::Current().TemporaryFolder();

    //auto folder = KnownFolders::GetFolderForUserAsync(nullptr, KnownFolderId::Objects3D);
    //folder->CreateFileAsync("testfile.txt");
    //StorageFile newFile{ folder.CreateFileAsync(desiredName, Windows::Storage::CreationCollisionOption::FailIfExists) };

    auto const meshCollection = m_meshRenderer->GetMeshCollection();
    
    String^ folderPath = ApplicationData::Current->LocalFolder->Path;
    //std::wstring const folderW(folderPath->Begin());

    // set path name to current time
    //wchar_t m_datetime_c[200];
    //const std::time_t now = std::time(nullptr);
    //std::tm tm;
    //std::wcsftime(m_datetime_c, sizeof(m_datetime_c), L"%F-%H%M%S", &tm);
    //std::wstring datepath(m_datetime_c);
    //auto test = std::string(datepath.begin(), datepath.end());

    String^ desiredName = "spatial_mesh.obj";
    auto createFileTask = create_task(localFolder->CreateFileAsync(desiredName, Windows::Storage::CreationCollisionOption::ReplaceExisting));
    createFileTask.then([meshCollection](StorageFile^ newFile)
    {
        auto platformPath = newFile->Path;
        std::wstring const pathW(platformPath->Begin());
        std::string const pathS(pathW.begin(), pathW.end());

        std::ofstream fileOut(pathS, std::ios::out);
        auto const numberOfMeshes = meshCollection.size();

        fileOut << "# " << numberOfMeshes << " mesh objects\n";
        int objCount = 1;
        int index_offset = 1;
        for (auto& pair : meshCollection) {
            auto const positions = pair.second.m_positions;
            auto const normals = pair.second.m_normals;
            auto const indices = pair.second.m_indices;
            fileOut << "o mesh_" << objCount << "\n";
            ...
            for (int i = 0; i < indices.size(); i += 3) {
                int const i1 = indices[i] + index_offset;
                int const i2 = indices[i + 1] + index_offset;
                int const i3 = indices[i + 2] + index_offset;
                int const n_index = (i / 3) + index_offset;

                fileOut << "f " << i1 << "//" << n_index << " "
                    << i2 << "//" << n_index << " "
                    << i3 << "//" << n_index << "\n";
        ...
        //create_task(newFile->OpenAsync(FileAccessMode::ReadWrite)).then([newFile, meshCollection](IOutputStream^ outputStream)
        //{
            ...
            //std::string num = std::to_string((int)meshCollection.size());

            //std::wstring w_str = std::wstring(num.begin(), num.end());
            //const wchar_t* w_chars = w_str.c_str();
            //auto help = ref new Platform::String(w_chars, w_str.length());

            //dataWriter->WriteString("Meshes: ");
            //double test2 = 2.3;
            //std::wstring s_1 = std::to_wstring(test2);
            //auto platString = ref new Platform::String(s_1.c_str(), s_1.length());

            //dataWriter->WriteString(platString);
            //dataWriter->WriteString("\n");
            
            //for (unsigned int i = 0; i < meshCollection.size(); i++) {
            //    dataWriter->WriteString("v  ");
            //    std::wstring s = std::to_wstring(meshCollection[i][0]);
            //    auto plat_s = ref new Platform::String(s.c_str(), s.length());
            //    dataWriter->WriteString(plat_s);
            //    dataWriter->WriteString(" ");
            //    s = std::to_wstring(meshCollection[i][1]);
            //    plat_s = ref new Platform::String(s.c_str(), s.length());
            //    dataWriter->WriteString(plat_s);
            //    dataWriter->WriteString(" ");
            //    s = std::to_wstring(meshCollection[i][2]);
            //    plat_s = ref new Platform::String(s.c_str(), s.length());
            //    dataWriter->WriteString(plat_s);
            //    dataWriter->WriteString("\n");
