...
void RealtimeSurfaceMeshRenderer::CreateDeviceDependentResources()
{
    ...
    auto finishLoadingTask = shaderTaskGroup.then([this]() {
        ...
        for (auto& iter : m_meshCollection)
        {
            ...
            iter.second.CreateDeviceDependentResources(m_deviceResources->GetD3DDevice(), m_worldCoordSystem);
        }
...
std::map<Platform::Guid, MeshData> RealtimeSurfaceMeshRenderer::GetMeshCollection()
{
    //std::lock_guard<std::mutex> guard(m_meshCollectionLock);
    std::map<Platform::Guid, MeshData> meshes;
    for (auto& pair : m_meshCollection)
    {
        meshes.insert({pair.first, pair.second.m_meshData});
        //meshes.push_back(pos);
    }
